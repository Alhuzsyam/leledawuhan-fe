...
const Relay = () => {
  ...
  const loadRelaysFromStorage = () => {
    try {
      ...
      console.error("Error loading relays to localStorage:", error);
    }
    return [];
  };
  ...
  const fetchRelays = async () => {
    ...
    try {
      ...
      if (storedRelays.length > 0) {
        ...
        showNotification(
          "Menggunakan data offline. Koneksi server bermasalah.",
          "error"
        );
      } else {
        setRelays([]);
  ...
  const updateSingleRelayValue = async (code, val, relayId) => {
    try {
      console.log("Updating single relay value:", {
        code,
        val,
        relayId,
    ...
    try {
      ...
      if (result.success) {
        const updatedRelays = relays.map((r) =>
          r.id === relay.id ? { ...r, val: newVal } : r
        );
        saveRelaysAfterAction(updatedRelays);
        showNotification(
          `Relay ${relay.code} berhasil ${
            newVal ? "diaktifkan" : "dinonaktifkan"
          }`
        );
      } else {
        showNotification(
          "Gagal mengubah nilai relay: " + result.error,
          "error"
        );
      }
    } catch (error) {
  ...
  const handleBulkRelayChange = async (activate) => {
    try {
      setApiLoading(true);
      const selectedRelayObjects = selectedRelays
        .map((relayId) => relays.find((r) => r.id === relayId))
        .filter(Boolean);
      console.log(
      ...
      for (const relay of selectedRelayObjects) {
        try {
          ...
          const result = await updateSingleRelayValue(
            relay.code,
            activate,
            relay.id
          );
      ...
      if (successCount > 0) {
        const updatedRelays = relays.map((relay) => {
          const successResult = results.find(
            (r) => r.success && r.relayId === relay.id
          );
          if (successResult) {
      ...
      if (successCount > 0 && failureCount === 0) {
        showNotification(
          `Semua ${successCount} relay berhasil ${
        ...
        showNotification(
          `Semua relay gagal diubah. Periksa koneksi dan coba lagi.`,
          "error"
        );
      }
      setSelectedRelays([]);
  ...
  const getAvailablePools = () => {
    return pools.filter(
      (pool) => !relays.some((relay) => relay.code === pool.code)
    );
  };
            ...
            <div className="bg-white rounded-2xl p-6 shadow-xl animate-fadeInUp delay-400 card-hover border border-gray-200">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-800">
                    {relays.length > 0
                      ? Math.round((activeRelaysCount / relays.length) * 100)
                      : 0}
                    %
                  </p>
                </div>
              ...
              {getAvailablePools().length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <h4 className="text-lg font-semibold text-gray-800 mb-2">
                    Semua Kolam Sudah Memiliki Relay
                  </h4>
                  <p className="text-gray-600">
                    Tidak ada kolam yang tersedia untuk ditambahkan relay baru.
                  </p>
                </div>
              ) : (
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <select
                        value={newRelayVal}
                        onChange={(e) =>
                          setNewRelayVal(e.target.value === "true")
                        }
                        className="w-full px-4 py-3 border-2 border-lime-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-lime-200 focus:border-lime-400 transition-all duration-300 bg-white"
                      >
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 mt-8">
                    <button
                      ...
                      disabled={apiLoading}
                      className="w-full sm:w-auto bg-gradient-to-r from-lime-400 to-green-500 hover:from-lime-500 hover:to-green-600 disabled:from-gray-300 disabled:to-gray-400 text-white px-8 py-3 rounded-xl transition-all duration-300 font-semibold shadow-lg hover:shadow-xl transform hover:scale-105 btn-ripple"
                    >
                      {apiLoading ? "Menyimpan..." : "Simpan Relay"}
                    </button>
                    <button
                      onClick={() => setShowAddForm(false)}
                      className="w-full sm:w-auto bg-gray-200 hover:bg-gray-300 text-gray-700 px-8 py-3 rounded-xl transition-all duration-300 font-semibold shadow-lg hover:shadow-xl transform hover:scale-105"
                    >
                      Batal
                    </button>
                  </div>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-green-800 font-bold text-lg">
                    {selectedRelays.length} relay dipilih
                  </span>
                </div>
                    ...
                    <div className="p-6 relative z-10">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                        <button
                          ...
                          disabled={apiLoading}
                          className={`w-full py-3 rounded-xl text-sm font-semibold transition-all duration-300 disabled:opacity-50 shadow-lg hover:shadow-xl transform hover:scale-105 btn-ripple flex items-center justify-center ${
                            relay.val
                              ? "bg-gradient-to-r from-orange-300 to-red-500 text-white hover:from-orange-500 hover:to-red-600"
